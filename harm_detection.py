...
try:
    ...
    DISCRIMINATIVE_CLASSES = [44, 77] # Id's taken from mscoco_label_map.pbtxt
...
def danger_detector(output_dict):
    """Identify objects in dangerous positions.
    
    Given the current output from the model, extract the approximate height and
    width of the detected objects from the given bounding boxes. Then determine 
    if the object is in a potentially dangerous position or not.
    ...
    if detections.size > 0:
        for i in detections:
            # Get the height and width of the detected object
            height = output_dict['detection_boxes'][i][2] - output_dict['detection_boxes'][i][0]
            width = output_dict['detection_boxes'][i].flatten()[3] - output_dict['detection_boxes'][i][1]
            if width > height:
                # Retrieve class name
                class_name = coco_classes_dict[str(output_dict['detection_classes'][i])]
                # Building the informative message
                DangerSituation.data_dict['class'] = class_name
                DangerSituation.data_dict['msg'] = str("Detected {} in possible \
                    dangerous disposal".format(class_name))
                DangerSituation.data_dict['confidence'] = str(output_dict['detection_scores'][i])
                message.append(json.loads(json.dumps(DangerSituation.data_dict)))
                # Load the message in the static variable for future use
                DangerSituation.message = str(message).replace("'", '"')
                print("Detected {} in possible dangerous disposal"
                    .format(class_name))
...
class HTTPhandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if 'img' in self.path:
            ...
            content = DangerSituation.img
            self.wfile.write(content[1].tobytes())
        elif 'alarms' in self.path:
            self._set_headers()
            message = DangerSituation.message
        ...
        with detection_graph.as_default():
            with tf.Session(graph=detection_graph) as sess:
                while True:  
                    ...
                    danger_detector(output)
                    cv2.imshow('Video', cv2.cvtColor(processed_image, cv2.COLOR_BGR2RGB))
                    DangerSituation.img = cv2.imencode('.jpeg', cv2.cvtColor(processed_image, cv2.COLOR_BGR2RGB))
